  ...
  return (
    <main className="arcade">
      ...
      <section className="game-section" aria-label="Geometry Conflict arcade game">
        ...
        <div className={`game-shell ${fullscreen ? 'fullscreen' : ''}`} ref={shell}>
          ...
          <div className={`arena ${active ? 'active-arena' : ''} ${game.status === 'reward' ? 'reward-arena' : ''}`} id="arena">
            ...
            {error ? <div className="start-screen error-screen" role="alert"><Crosshair size={36}/><h2>CONNECTION<br/><span>LOST.</span></h2><p>{error}</p><button className="start-button" onClick={() => window.location.reload()}><RotateCcw size={17}/> RELOAD ARENA</button></div> : game.status === 'reward' ? <RewardScreen game={game} onChoose={id => engine.current?.chooseReward(id)} onReroll={() => engine.current?.rerollRewards()}/> : game.status === 'ready' ? (
              <div className="start-screen"><div className="ready-pill"><span/> 10 WEAPONS · 36 RELICS · ONE RUN</div><h2>ENTER<br/>THE <span>GRID.</span></h2><p>Survive the wave. Choose your upgrades. Build something lethal.</p><button className="start-button" onClick={start} disabled={!ready}><Play size={17} fill="currentColor"/> {ready ? 'START GAME' : 'CONNECTING…'} <ArrowUpRight size={20}/></button><div className="enter-hint desktop-hint">or press <kbd>ENTER</kbd></div><div className="enter-hint mobile-hint">Two thumbs. Total control.</div></div>
            ) : game.status === 'paused' ? (
              ...
              <div className="start-screen over-screen"><div className="ready-pill">{game.score > 0 && game.score >= game.best ? 'PERSONAL BEST' : 'RUN COMPLETE'}</div><h2>SIGNAL<br/><span>LOST.</span></h2><div className="final-weapon" style={{ color: game.weapon.color }}>{game.weapon.name} · LV. {game.weapon.level} · {game.relics.length} RELICS</div><div className="end-score">{scoreText(game.score)}<span>FINAL SCORE</span></div><div className="run-stats"><span>WAVE <b>{game.wave}</b></span><span>KILLS <b>{game.kills}</b></span><span>TIME <b>{timeText(game.time)}</b></span></div><button className="start-button" onClick={start}><RotateCcw size={17}/> PLAY AGAIN <ArrowUpRight size={20}/></button><div className="enter-hint desktop-hint">or press <kbd>ENTER</kbd></div></div>
            ) : null}
          ...
          <div className="loadout-bar"><div style={{ color: game.weapon.color }}><Crosshair size={15}/><b>{game.weapon.name}</b><span>LV. {game.weapon.level}</span><small style={{ color: RARITY_COLOR[game.weapon.rarity] }}>{game.weapon.rarity}</small></div><div className="run-resources"><Layers size={14}/>{game.relics.length} RELICS<i/><RotateCcw size={13}/>{game.rerolls} REROLLS</div></div>
        ...
        {game.relics.length > 0 && <details className="run-build" onToggle={e => { if (e.currentTarget.open && game.status === 'playing') engine.current?.togglePause(); }}><summary><Layers size={16}/> YOUR RUN BUILD <span>{game.relics.length} RELICS</span></summary>{game.weapon.synergies.length > 0 && <div className="build-synergies"><Sparkles size={16}/>{game.weapon.synergies.join(' / ')}</div>}<div className="owned-relics">{game.relics.map((relic, i) => <div key={i}><b style={{ color: RARITY_COLOR[relic.rarity] }}>{relic.name}<small>{relic.rarity}</small></b><p>{relic.description}</p></div>)}</div></details>}
      </section>
