...
export function registerGameTools(engine: GameEngine) {
  ...
  const tools: Tool[] = [
    { name: 'get_game_status', description: 'Read the current Geometry Conflict run: score, wave, lives, shields, both equipped weapons and their levels, relics, rerolls, and offered rewards.', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, untrustedContentHint: false }, execute(input) { if (!input || typeof input !== 'object' || Array.isArray(input) || Object.keys(input).length) throw new Error('Expected an empty object.'); return engine.model.snapshot(); } },
