...
test('every weapon pair handles a crowded arena with combined projectile and elemental effects', () => {
  const pairs = WEAPONS.flatMap((a, i) => WEAPONS.slice(i + 1).map(b => [a, b]));
  for (const pair of pairs) {
    const g = new GameModel(() => .5); g.start(); g.weapons = pair.map(weapon => ({ id: weapon.id, level: 8, rarity: 'legendary' })); g.invulnerable = 10000;
